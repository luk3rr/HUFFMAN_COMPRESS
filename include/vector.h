...
template<typename typeT>
Vector<typeT>::Vector(unsigned int startSize) {
    this->m_capacity = startSize;
    this->m_size = 0;
    this->m_elements = new typeT[this->m_capacity];
}
...
template<typename typeT>
typeT Vector<typeT>::PopBack() {
    if (this->IsEmpty())
        throw vecexcpt::VectorIsEmpty();

    this->m_size--;
    return this->m_elements[m_size];
}
